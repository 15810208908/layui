...
layui.define(['lay', 'laytpl', 'laypage', 'form', 'util'], function (exports) {
  ...
  var parseTempData = function (obj) {
    ...
    // 获取模板内容
    if (templet) {
      content = typeof templet === 'function' ? templet.call(item3, obj.tplData, obj.obj) : laytpl(resolveTplStr(templet) || String(content)).render($.extend({ LAY_COL: item3 }, obj.tplData));
    }
  ...
  var Class = function (options) {
    ...
    layui.each(that.config.cols, function (idx, rows) {
      layui.each(rows, function () {
  ...
  Class.prototype.render = function (type) {
    ...
    var id = options.id = 'id' in options ? options.id : (options.elem.attr('id') || that.index);
